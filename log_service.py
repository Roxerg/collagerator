...
class LogService: 
    ...
    def __init__(self):
        ...
        requests_formatter = logging.Formatter('%(asctime)s | %(message)s %(guild)s %(username)s | %(command)s %(period)s %(dimensions)s %(response_code)s %(response)s %(listof)s %(count)s')  
    ...
    def build_extras(self, command = '', username='', dimensions='', period='', response_code='', response='', guild='', count='', listof=''):
        return {
            'command' : command,
    ...
    def response(self, command, username, statuscode, response):
        ...
        if statuscode != 1:
            self.errors_logger.error('FAIL MESSAGE', 
                extra=self.build_extras(response=response)
            )
